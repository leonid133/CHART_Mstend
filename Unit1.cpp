...
__fastcall TForm1::TForm1(TComponent* Owner)
        ...
        , pos(0)
{
   Edit10->Text = 100 - 100 * float(TrackBar1->Position)/float(TrackBar1->Max);
}
      ...
      const AnsiString Str)
{
   ...
   float ls = 1.35962162*ff1*ff3*0.5/M_PI/60.;
   Edit8->Text = ls;
      ...
      const AnsiString Str)
{
   ...
   static float af;
   sscanf (Str.c_str(), "%f %f %f %f %f", &a1, &a2, &a3, &a4, &a5);

   Series1->AddXY(Series1->Count()+1, a1);
   Edit1->Text = a1;

   Series2->AddXY(Series2->Count()+1, a2);
   Edit2->Text = a2;
   af = af+(a2-af)*0.1;

   Series3->AddXY(Series3->Count()+1, a3);
   Edit3->Text = a3;
   Edit5->Text = a1*a3*0.5/M_PI/60.;

   Series4->AddXY(Series4->Count()+1, a4);
   Edit4->Text = a4;
   Sumf4 += a4;

   Series5->AddXY(Series5->Count()+1, a5);
   Edit7->Text = a5;

   float ls = 0.00135962162*a1*a3*0.5/M_PI/60.;
   Edit8->Text = ls;

   float rash = 0.72 * a5 * ls * 60.;
   Edit9->Text = rash;
...
{ 
   Chart1->BottomAxis->Scroll(-(Chart1->BottomAxis->Minimum),false);
   Chart2->BottomAxis->Scroll(-(Chart2->BottomAxis->Minimum),false);
   Chart3->BottomAxis->Scroll(-(Chart3->BottomAxis->Minimum),false);
   Chart4->BottomAxis->Scroll(-(Chart4->BottomAxis->Minimum),false);
   Chart5->BottomAxis->Scroll(-(Chart5->BottomAxis->Minimum),false);
   Series1->Clear();
   Series2->Clear();
   Series3->Clear();
   Series4->Clear();
...
void __fastcall TForm1::FormResize(TObject *Sender)
{
   ...
   Edit3->Left = Chart1->Left + Chart1->Width + 10 + 30;
   Edit3->Top = Chart3->Top + 2;

   Chart5->Height = h;
   Chart5->Top = Chart3->Top+Chart3->Height;
   Chart5->Width = ClientWidth - (Edit1->Width + 20);
   Edit7->Left = Chart1->Left + Chart1->Width + 10 + 30;
   Edit7->Top = Chart5->Top + 2 + 5;
   ...
   Edit9->Top = Edit4->Top + Edit4->Height + 30;

   Shape1->Width = 1;
   Shape1->Top = Chart1->Top;
   Shape1->Height = ClientHeight;

   TrackBar1->Left = Chart3->Left + Chart3->Width + 2;
   TrackBar1->Top = Chart3->Top;
   TrackBar1->Height = Chart3->Height + Chart5->Height;
   TrackBar1->Width = 30;

   Edit10->Top = TrackBar1->Top + TrackBar1->Height - Edit10->Height;
   Edit10->Left = Chart1->Left + Chart1->Width + 10 + 30;

//   About->Top = 2;
//   About->Left = ClientWidth - About->Width - 8;
}
...
{
   int r1 = 100 - TrackBar1->Position;

   String txt; txt.sprintf("%d", r1);
   String r = "x"+txt;
   if (ComPort->Connected)
      ComPort->WriteStr(r.c_str());
   Edit10->Text = txt;
}
//---------------------------------------------------------------------------
